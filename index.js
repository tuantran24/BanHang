...
let getData = async () => {
    ...
    try {
        ...
        let response = await fetch(`index.json${searchInputTxt}`);
...
const append =(data)=>{
   ...
   sup.innerHTML = foodArr.length;
    ...
    let price = Math.floor(Math.random() * (300000 - 50000 + 1)) + 50000;
    ...
    data.forEach(el => {
        ...
        p2.addEventListener('click', function(e) {
           let timer = setInterval(() => {
            ...
            let foodObj = {
                name: el.strMeal,
                price: parseInt(price.innerHTML) + ".000 VND",
                img: el.strMealThumb
